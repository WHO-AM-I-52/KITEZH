...
# ║  FIX: ожидание иконки только в tray-режиме (KITEZH_TRAY=1)  ║
# ║       в обычном режиме консоль скрывается сразу             ║
# ╚══════════════════════════════════════════════════════════════╝
...
def hide_console() -> bool:
    ...
    Поведение зависит от режима запуска:
      - KITEZH_TRAY=1 (трей-режим): ждёт готовности иконки до 5 сек.
        Если иконка не появилась — отменяет скрытие (консоль не пропадёт
        без возможности вернуться).
      - Обычный режим (без трея): скрывает сразу, без ожидания.
        Вернуть консоль можно через POST /api/console/show из браузера.

    Возвращает True если успешно, False если окно не найдено
    или трей не успел запуститься (только в tray-режиме).
    """
    global _console_visible

    tray_mode = os.environ.get('KITEZH_TRAY', '0') == '1'

    if tray_mode and not _tray_ready.is_set():
