...
:: ────────────────────────────────────────────────────────────────────────────
:: Запуск сервера через вспомогательный скрипт run_server.py.
::
:: run_server.py записывает свой PID в _server.pid, запускает Flask,
:: затем ждёт завершения и возвращает код выхода:
::   0 — обычная остановка (Ctrl+C пользователем)
::   42 — нужен авторестарт (обновление через браузер)
::
:: Батник ожидает завершения run_server.py (нет Y/N?),
:: читает код выхода и решает перезапускаться или нет.
:: ────────────────────────────────────────────────────────────────────────────
:start_server
