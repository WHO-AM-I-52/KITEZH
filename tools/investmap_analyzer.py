...
# ---------------------------------------------------------------------------
# Значения родительских полей, при которых зависимые поля
# физически недоступны на портале и не должны считаться незаполненными.
# ---------------------------------------------------------------------------
...
def _score_portal(data: dict, fmt: str) -> tuple[int, list[str]]:
    ...
    for key, only_fmt in PORTAL_FIELDS:
        ...
        # Зависимые поля: пропускаем если родительское поле неактивно
        if key == 'Наименование объекта преференциального режима':
            ...
            if parent in PREF_INACTIVE_VALUES:
                continue  # поле неактивно на портале при Отсутствует
            ...
            if parent in INFRA_INACTIVE_VALUES:
                continue  # поле неактивно на портале при Без льгот
