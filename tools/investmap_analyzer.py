...
# v1.5.2: ОКВЭД 'Нет ограничений' не засчитывается (портал ожидает реальные коды)
# ---------------------------------------------------------------------------
PORTAL_FIELDS = [
    ('Статус',                                              None),
...
# Портал не засчитывает такие ответы — ожидается содержательное заполнение.
...
def _is_irrelevant(key: str, val: str) -> bool:
    """
    v1.5.2: проверяет, является ли значение поля нерелевантным
    для портала (содержательно пустым ответом типа 'Нет ограничений').
...
def _is_filled_portal(val: str, key: str, data: dict) -> bool:
    ...
    Правила:
    1. «Порядок определения стоимости» = 'отсутствует' → засчитывается.
    2. «Описание процедуры подачи заявки» / «Перечень документов» = 'не применимо'
       → засчитывается ТОЛЬКО если «Стоимость объекта» заполнена.
    3. v1.5.2: поля из IRRELEVANT_FIELD_VALUES с нерелевантным значением
       → НЕ засчитываются (портал ожидает содержательный ответ).
    4. Во всех остальных случаях — стандартный _is_filled.
    """
    if not _is_filled(val):
        return False
