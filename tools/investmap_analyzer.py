...
FIELD_INSTRUCTIONS = {
    ...
    'Наименование объекта преференциального режима': 'заполните наименование объекта преференциального режима',
    'Наименование объекта инфраструктуры поддержки': 'заполните наименование объекта инфраструктуры поддержки',
...
def _build_sms(площадка_id: str, площадка_name: str, total_portal: int, missing_portal: list[str]) -> str | None:
    if total_portal >= SMS_THRESHOLD or not missing_portal:
        return None
    ...
    lines.append(
        f'Для повышения показателя просим заполнить {len(missing_portal)} полей — '
