...
#         (ОТКАТАНО в v1.5.6 — портал принимает «Нет» как валидный ответ)
# v1.5.6: откат IRRELEVANT для «Наличие ж/д» и «Наличие парковки»
#         эталон 1521055: 79% (КИТЕЖ v1.5.5) → 82% (портал) → дельта закрыта
# ---------------------------------------------------------------------------
PORTAL_FIELDS = [
    ('Статус',                                              None),
...
# v1.5.6: ключи 'наличие ж/д' и 'наличие парковки' УДАЛЕНЫ —
#         портал принимает «Нет» как валидный ответ для этих полей
# ---------------------------------------------------------------------------
IRRELEVANT_FIELD_VALUES: dict[str, set[str]] = {
    ...
    # v1.5.5: 'наличие ж/д' и 'наличие парковки' были здесь с {'нет'}
    # v1.5.6: УДАЛЕНЫ — портал засчитывает «Нет» как валидный ответ
    # (доказано на эталоне 1521055: портал 82% vs КИТЕЖ 79%)
}
...
def _is_irrelevant(key: str, val: str) -> bool:
    ...
    v1.5.6: 'наличие ж/д' и 'наличие парковки' УДАЛЕНЫ из IRRELEVANT —
            портал засчитывает «Нет» как валидный ответ для этих полей.
...
def _is_filled_portal(val: str, key: str, data: dict) -> bool:
    ...
    2. v1.5.2+: поля из IRRELEVANT_FIELD_VALUES с нерелевантным значением
       ...
       v1.5.6: «Нет» в «Наличие ж/д» и «Наличие парковки» ЗАСЧИТЫВАЕТСЯ
               (откат v1.5.5 — портал принимает «Нет» как валидный ответ).
