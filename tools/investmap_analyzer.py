...
#         (эталон 1521055 с дельтой +1% подтверждает).
# v1.5.4: _find() теперь применяет _strip_html() к значению перед возвратом,
#         поэтому '<p>Не применимо.</p>' корректно распознаётся как нерелевантное.
# Ключ словаря — фрагмент названия поля (поиск через _norm/in),
# значение — множество нерелевантных строк в нижнем регистре.
# ---------------------------------------------------------------------------
...
def _is_irrelevant(key: str, val: str) -> bool:
    ...
    v1.5.4: val уже очищен от HTML через _find/_strip_html, поэтому
            '<p>Не применимо.</p>' корректно распознаётся здесь как 'не применимо.'.
...
def _is_filled_portal(val: str, key: str, data: dict) -> bool:
    ...
    2. v1.5.2+: поля из IRRELEVANT_FIELD_VALUES с нерелевантным значением
       ...
       v1.5.3: в IRRELEVANT_FIELD_VALUES добавлены 'описание процедуры' и
               'перечень документов' — 'не применимо' в них НЕ засчитывается
               НИКОГДА (убрана зависимость от Стоимости объекта).
               Эталон 1521055 (дельта +1%) это подтверждает.
       v1.5.4: val уже очищен от HTML через _find(), поэтому HTML-обёртка
               '<p>Не применимо.</p>' корректно попадает в IRRELEVANT_FIELD_VALUES.
